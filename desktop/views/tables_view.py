...
class TablesView(QWidget):
    ...
    def _refresh_from_api(self):
        ...
        try:
            ...
            if not hasattr(self.api_client, 'access_token') or not self.api_client.access_token:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 401:
                    print("Token expired, falling back to database")
                self._refresh_from_db()  # Fallback
    ...
    def _refresh_from_db(self):
        """Refresh tables data from database (original method)"""
        ...
        for table in tables:
            if table.id in self.cards:
                print(f"Refreshing table {table.number}: status={table.status}")
