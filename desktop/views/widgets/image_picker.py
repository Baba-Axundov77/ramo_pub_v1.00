...
# Python 3.11+ uyğun — şəkil seçmə və göstərmə widgeti
...
def _copy_image(src: str, dest_dir: Path, prefix: str = "") -> str:
    """Şəkili dest_dir-ə kopyala, nisbi yolu qaytar."""
    src_path = Path(src)
    if src_path.suffix.lower() not in ALLOWED_EXT:
        raise ValueError(f"Dəstəklənməyən format: {src_path.suffix}")
...
def _make_pixmap(
    ...
    width: int,
    height: int,
    placeholder_icon: str = "🖼️",
    placeholder_text: str = "Şəkil yoxdur",
) -> QPixmap:
    """
    Şəkil faylından QPixmap yarat.
    Fayl yoxdursa — placeholder icon çək.
    """
...
class RoundedImageLabel(QLabel):
    """
    Yuvarlaq künclü şəkil label.
    image_path: fayl yolu (None → placeholder)
    """
    def __init__(
        self,
...
class ImagePickerWidget(QWidget):
    """
    Şəkil seç / sil / göstər widgeti.

    Siqnallar:
        image_changed(str)  — yeni şəkil yolu (boş string = silindi)
    """
    ...
    def __init__(
        ...
        placeholder_text: str = "Şəkil seçin",
        parent: Optional[QWidget] = None,
    ) -> None:
        super().__init__(parent)
    ...
    def _build(self) -> None:
        ...
        self.select_btn = QPushButton("📂  Şəkil seç")
        ...
        # Fayl adı
        self.file_label = QLabel("Şəkil seçilməyib")
    ...
    def _pick_file(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            ...
            "Şəkil seç",
            str(Path.home()),
            "Şəkil faylları (*.jpg *.jpeg *.png *.webp *.bmp)"
        )
        if not path:
            return
        try:
            ...
            QMessageBox.warning(self, "Xəta", f"Şəkil kopyalana bilmədi:\n{e}")
    ...
    def _refresh_preview(self) -> None:
        ...
        if self._current:
            ...
            self.file_label.setText("Şəkil seçilməyib")
            self.file_label.setStyleSheet("color: #6A6A8A; font-size: 10px;")
