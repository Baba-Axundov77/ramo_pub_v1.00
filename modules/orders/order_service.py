...
class OrderService:
    ...
    def add_item(self, db: Session, order_id: int, menu_item_id: int,
        ...
        try:
            ...
            db.rollback()
            return False, f"Sifarişə əlavə edilərkən xəta: {str(e)}"
    ...
    def remove_item(self, db: Session, order_item_id: int):
        try:
            ...
            db.rollback()
            return False, f"Sifariş elementi silinərkən xəta: {str(e)}"
    ...
    def update_item_qty(self, db: Session, order_item_id: int, quantity: int):
        try:
            ...
            db.rollback()
            return False, f"Miqdar dəyişdirilərkən xəta: {str(e)}"
    ...
    def update_status(self, db: Session, order_id: int, status: str):
        try:
            ...
            db.rollback()
            return False, f"Status dəyişdirilərkən xəta: {str(e)}"
    ...
    def cancel_order(self, db: Session, order_id: int, reason: str = None):
        try:
            ...
            db.rollback()
            return False, f"Sifariş ləğv edilərkən xəta: {str(e)}"
    ...
    def apply_discount(self, db: Session, order_id: int, discount_amount: float):
        try:
            ...
            db.rollback()
            return False, f"Endirim tətbiq edilərkən xəta: {str(e)}"
