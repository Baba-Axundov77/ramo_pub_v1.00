...
class AdvancedOrderService:
    ...
    def get_order_history_paginated(self, db: Session, customer_id: Optional[int] = None, 
                                   table_id: Optional[int] = None,
                                   date_from: Optional[datetime] = None,
                                   date_to: Optional[datetime] = None,
                                   per_page: int = 20, offset: int = 0) -> Tuple[bool, Dict]:
        """Get comprehensive order history with pagination"""
        try:
            # Get total count
            ...
            if customer_id:
                count_query = count_query.filter(Order.customer_id == customer_id)
            if table_id:
                count_query = count_query.filter(Order.table_id == table_id)
            if date_from:
                count_query = count_query.filter(Order.created_at >= date_from)
            if date_to:
                count_query = count_query.filter(Order.created_at <= date_to)
            ...
            return False, f"Failed to fetch order history: {str(e)}"
