...
# Import config for API access (fallback only)
try:
...
# Always use localhost for desktop API calls
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, user, db, auth_service, theme: str = "dark", api_client=None):
        super().__init__()
        ...
        self._services = {}
        self._views = {}
        self.api_client = api_client  # Set API client immediately

        self.setWindowTitle(f"Ramo Pub & TeaHouse — {user.full_name}")
        self.setMinimumSize(1280, 780)
        self.showMaximized()

        # Add API client for real-time sync - always use 127.0.0.1 for desktop
        from src.core.modules.desktop.api_client import DesktopAPIClient
        # Don't create new API client, use the one from login
        # self.api_client = DesktopAPIClient(f"http://127.0.0.1:5000")
        
        # Add background threads for real-time updates
        from src.core.modules.desktop.dashboard_thread import DashboardDataThread
        # API client will be set from main.py after login
    ...
    def set_api_client(self, api_client):
        """Set API client after login and start background threads"""
        # Set API client if not already set
        if not hasattr(self, 'api_client') or self.api_client is None:
            self.api_client = api_client
        
        # Setup pages only if not already setup
        if not hasattr(self, '_pages_setup'):
            self._setup_pages()
            self._pages_setup = True
        
        # Set API client for tables view if it exists
        if "tables" in self._views and hasattr(self._views["tables"], 'set_api_client'):
            self._views["tables"].set_api_client(api_client)
        
        # Start background threads now that we have API client
        from src.core.modules.desktop.dashboard_thread import DashboardDataThread
        ...
        if hasattr(self.api_client, 'access_token') and self.api_client.access_token:
            print("API token available for real-time updates")
        else:
            print("No API token available - real-time updates disabled")

    def _on_dashboard_update(self, data):
        """Handle real-time dashboard updates from API"""
        try:
            # Update all views with fresh data using specific methods
            for view_name, view in self._views.items():
                refreshed = False
                
                # Tables view specific refresh
                if view_name == "tables" and hasattr(view, '_refresh'):
                    view._refresh()
                    refreshed = True
                    
                # Orders view specific refresh
                elif view_name == "orders":
                    if hasattr(view, '_load_active_orders'):
                        view._load_active_orders()
                        refreshed = True
                    
                # Menu view specific refresh
                elif view_name == "menu":
                    if hasattr(view, '_load_categories'):
                        view._load_categories()
                        refreshed = True
                    if hasattr(view, '_load_items'):
                        view._load_items()
                        refreshed = True
                
                # Dashboard view
                elif view_name == "dashboard" and hasattr(view, 'refresh_data'):
                    view.refresh_data()
                    refreshed = True
                ...
                if not refreshed:
                    if hasattr(view, 'refresh_data'):
                        view.refresh_data()
                    elif hasattr(view, 'refresh_tables'):
                        view.refresh_tables()
                        ...
                        # Last resort: try to reload the view
                        try:
                            if hasattr(view, '_refresh_data'):
                                view._refresh_data()
                            elif hasattr(view, '_load_data'):
                                view._load_data()
                        except:
                            pass
                        
            print(f"Updated {len(self._views)} views with fresh data")
                    
        except Exception as e:
            print(f"Error updating dashboard: {e}")

    def _manual_refresh(self):
        """Manual refresh for current view and all views"""
        try:
            current_page = self.stack.currentWidget()
            
            # For now, skip API calls and refresh views directly
            # TODO: Fix API authentication and re-enable API calls
            print("Manual refresh - using database fallback")
            
            # Update all views with fresh data
            self._on_dashboard_update({})
                    
        except Exception as e:
            print(f"Manual refresh error: {e}")
            # Try to refresh current view as fallback
            try:
                current_page = self.stack.currentWidget()
                self._refresh_current_view(current_page)
            except:
                pass

    def _refresh_current_view(self, view):
        """Refresh only the current view using specific methods"""
        refreshed = False
        
        # Find which view this is
        view_name = None
    ...
    def closeEvent(self, event):
        ...
        try:
            if hasattr(self, 'dashboard_thread'):
                ...
                self.dashboard_thread.wait(3000)  # Wait up to 3 seconds
    ...
    def _init_services(self):
        from src.core.modules.tables.table_service import TableService
        from src.core.modules.menu.menu_service import MenuService
        from src.core.modules.orders.order_service import OrderService
        from src.core.modules.orders.workflow_service import OrderWorkflowService
        ...
        from src.core.modules.reservation.reservation_service import ReservationService
        from src.core.modules.staff.staff_service import StaffService
        from src.core.modules.reports.report_service import ReportService
        from src.core.modules.loyalty.loyalty_service import LoyaltyService
        from src.core.modules.printer.printer_service import PrinterService
        from src.core.modules.orders.kitchen_service import kitchen_service
    ...
    def _build_ui(self):
        ...
        # Add refresh button
        refresh_btn = SidebarButton("🔄", "Yenilə")
        refresh_btn.setStyleSheet(self._sidebar_btn_style())
        refresh_btn.clicked.connect(self._manual_refresh)
        sidebar_v.addWidget(refresh_btn)
    ...
    def _setup_pages(self):
        from src.desktop.views.tables_view import TablesView
        from src.desktop.views.orders_view import OrdersView
        from src.desktop.views.menu_view import MenuView
        from src.desktop.views.inventory_view import InventoryView
        from src.desktop.views.reservation_view import ReservationView
        from src.desktop.views.staff_view import StaffView
        from src.desktop.views.reports_view import ReportsView

        # ── Masalar ───────────────────────────────────────────────────────────
        tables_view = TablesView(self.db, self._services["tables"], self.auth, self.api_client)
        ...
        # ── Loyallıq ──────────────────────────────────────────────────────────
        from src.desktop.views.loyalty_view import LoyaltyView
        self._views["loyalty"] = LoyaltyView(
            self.db, self._services["loyalty"], self.auth
        )

        # ── Kassa ─────────────────────────────────────────────────────────────
        from src.desktop.views.pos_dashboard_view import POSDashboardView
        ...
        # ── Mətbəx ───────────────────────────────────────────────────────────
        from src.desktop.views.kitchen_view import KitchenView
    ...
    def _open_payment(self, order):
        from src.desktop.views.pos_view import PaymentView
